(function (root, factory) {
    ...
    function createWrapper(obj, parent, options) {
        return {
            ...
            addAbove: function(newObj) {
                if (!parent) {
                    throw new Error('Cannot add above the root item');
                }
                var parentsItems = getOrCreateChildItems(parent, options.childrenProp);
                var thisItemIndex = parentsItems.indexOf(obj);
                if (thisItemIndex < 0) {
                    throw new Error('Could not find item in parent items');
                }
                parentsItems.splice(thisItemIndex, 0, newObj);
                options.onAdd(parent, thisItemIndex, newObj);
            },
            addBelow: function(newObj) {
                if (!parent) {
                ...
                parentsItems.splice(thisItemIndex + 1, 0, newObj);
                options.onAdd(parent, thisItemIndex + 1, newObj);
            },
